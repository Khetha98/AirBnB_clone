"""Unittest for the base_model module."""
import json
import os
import time
import unittest
...
class TestBase(unittest.TestCase):
    """The test cases for the Base class.
    """
    ...
    def test_initialization_positive(self):
        """It test passing cases for BaseModel initialization.
        """
        b1 = BaseModel()
        b2_uuid = str(uuid.uuid4())
        b2 = BaseModel(id=b2_uuid, name="weeknd", album="Trilogy")
        self.assertIsInstance(b1.id, str)
        self.assertIsInstance(b2.id, str)
        self.assertEqual(b2_uuid, b2.id)
        self.assertEqual(b2.album, "Trilogy")
        self.assertEqual(b2.name, "weeknd")
    ...
    def test_save_storage(self):
        """It tests that storage.save() is called from save()."""
        b = BaseModel()
        b.save()
